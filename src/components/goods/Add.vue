...
export default {
  data() {
    return { // 添加商品的数据对象
      ...
      uploadURL: 'http://127.0.0.1:8888/api/private/v1/upload', // 上传图片的 URL 地址
      headerObj: { // 图片上传组件的headers请求头对象
        Authorization: window.sessionStorage.getItem('token')
      },
      previewPath: '',
      previewVisible: false // 预览图片窗口默认关闭
    }
  },
  created() {
    this.getCateList()
  ...
  methods: {
    ...
    // activeName——进入的标签页，oldActiveName——离开的标签页
    ...
    add() {
      this.$refs.addFormRef.validate(async valid => {
        ...
        // 执行添加的业务逻辑
        const form = lodash.cloneDeep(this.addForm) // 深拷贝
